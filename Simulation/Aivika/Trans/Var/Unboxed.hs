...
newVar :: (MonadDES m, Unboxed m a) => a -> Simulation m (Var m a)
newVar a =
  Simulation $ \r ->
...
varMemo :: (MonadDES m, Unboxed m a) => Var m a -> Dynamics m a
varMemo v =
...
readVar :: (MonadDES m, Unboxed m a) => Var m a -> Event m a
readVar v = 
  Event $ \p ->
  do let xs = varXS v
         ys = varYS v
...
writeVar :: (MonadDES m, Unboxed m a) => Var m a -> a -> Event m ()
writeVar v a =
  Event $ \p ->
  do let xs = varXS v
         ms = varMS v
...
modifyVar :: (MonadDES m, Unboxed m a) => Var m a -> (a -> a) -> Event m ()
modifyVar v f =
  Event $ \p ->
  do let xs = varXS v
         ms = varMS v
...
freezeVar :: (MonadDES m, Unboxed m a) => Var m a -> Event m (Array Int Double, Array Int a, Array Int a)
freezeVar v =
  Event $ \p ->
